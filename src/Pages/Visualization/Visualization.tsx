...
import "../../colors.css";
import { useRun } from '../../Components/Tutorial/RunContext';
import { fakeData } from "../../Components/Consts";
...
const Visualization: React.FC<iVisualization> = ({ colorInverted }) => {
  ...
  const updateFakeData = () => {
    const createProperty = fakeData?.results?.items?.map((content: any) => ({
      ...content,
      showTooltip: false,
    }));
    ...
    setSeparatedWords(fakeData?.results?.transcripts[0]?.transcript?.split(" "));
    setSketch(fakeData?.results?.transcripts[0]?.transcript);
    setBackup(fakeData.results.transcripts[0].transcript);
  };
